...
export function KingpinChatPanel() {
  return (
    <section className="flex min-h-[calc(100vh-128px)] flex-col rounded-[24px] border border-white/7 bg-[#151d29] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.28)]">
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="min-h-[420px] flex-1 space-y-4 overflow-hidden py-5">
          {chatMessages.map((message, index) => (
            <a
              href="#"
              key={index}
              className={`block w-full rounded-2xl border px-4 py-3 text-sm leading-6 ${
                message.tone === "assistant"
                  ? "max-w-[94%] border-[#2c3544] bg-[#252d3a] text-[#d8c79e]"
                  : "ml-auto max-w-[86%] border-[#283243] bg-[#2a3444] text-white/82"
              }`}
            >
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <div>
            ...
            <div className="space-y-2">
              {pendingActions.map((item) => (
                <a key={item.label} href="#" className="flex items-center justify-between rounded-xl border border-white/6 bg-[#141b26] px-4 py-2.5 text-sm text-white/70">
          ...
          <div>
            ...
            <div className="space-y-2">
              {quickTools.map((tool) => (
                <a key={tool} href="#" className="flex items-center justify-between rounded-xl border border-white/6 bg-[#141b26] px-4 py-2.5 text-sm text-white/70">
                  <span>{tool}</span>
